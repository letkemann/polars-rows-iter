...
fn create_from_dataframe_row_trait_impl(ctx: &Context) -> proc_macro2::TokenStream {
    ...
    let iter_create_list = ctx.fields_list.iter().map(|f| {
        ...
        quote! {
            ...
            let #ident_iter = Box::new(<Option<<#field_type as ::polars_rows_iter::IterFromColumn<#lifetime>>::RawInner> as ::polars_rows_iter::IterFromColumnRaw<#lifetime>>::create_iter(column)?);
